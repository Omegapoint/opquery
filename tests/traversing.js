...
test("parent([String])", function() {
	...
	equal( jQuery("#groups").parent().get(0).id, "ap", "Simple parent check" );
	equal( jQuery("#groups").parent("p").get(0).id, "ap", "Filtered parent check" );
	equal( jQuery("#groups").parent("div").length, 0, "Filtered parent check, no match" );
	equal( jQuery("#groups").parent("div, p").get(0).id, "ap", "Check for multiple filters" );
...
test("next([String])", function() {
	expect(6);
	equal( jQuery("#ap").next().get(0).id, "foo", "Simple next check" );
	equal( jQuery("<div>text<a id='element'></a></div>").contents().eq(0).next().attr("id"), "element", "Text node next check" );
	equal( jQuery("#ap").next("div").get(0).id, "foo", "Filtered next check" );
	equal( jQuery("#ap").next("p").length, 0, "Filtered next check, no match" );
	equal( jQuery("#ap").next("div, p").get(0).id, "foo", "Multiple filters" );
